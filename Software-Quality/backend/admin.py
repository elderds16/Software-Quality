...
def reset_admin_password_with_temporary_password(event):
    ...
    with Database() as (_, cur):
        try:
            ...
            print("Error: ", e)
            return CustomResponse(500, body.get("invokerUsername"), "Could not reset password", f"ERROR while updating user in database: {str(e)}", False)

    Log().log_activity(body.get("invokerUsername"), "Reset admin password", f"{body.get('invokerUsername')} reset password with a temporary password", False)
